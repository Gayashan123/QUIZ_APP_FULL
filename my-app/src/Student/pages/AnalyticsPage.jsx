...
export default function AnalyticsPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        // 2) Hydrate missing quiz details (subject, teacher, title)
        ...
        if (mounted) setAttempts(normalized);
      } catch (e) {
        console.error(e);
        if (mounted) setError("Unable to load analytics. Please try again.");
      } finally {
  ...
  const filteredByTime = useMemo(() => {
    ...
    return attempts.filter(a => 
      a.finishedAtTs && a.finishedAtTs >= cutoffDate.getTime()
    );
  }, [attempts, timeRange]);
  ...
  const analytics = useMemo(() => {
    ...
    const firstHalfAvg = half > 0 
      ? Math.round(sortedByDate.slice(0, half).reduce((sum, a) => sum + (a.scorePercent || 0), 0) / half)
      : 0;
    const secondHalfAvg = sortedByDate.length - half > 0
      ? Math.round(sortedByDate.slice(half).reduce((sum, a) => sum + (a.scorePercent || 0), 0) / (sortedByDate.length - half))
      : 0;
    ...
    const consistency = Math.round(100 - (Math.sqrt(variance) / 100 * 100));

    // Trend (sorted by finished date)
    ...
    for (const a of completed) {
      ...
      const entry = map.get(key) || { subject: key, total: 0, count: 0, best: 0, worst: 100 };
        ...
        <div className="p-6 md:p-8">
          ...
          <section className="mb-8">
            <div className="bg-gradient-to-tr from-indigo-600 via-purple-600 to-pink-500 text-white rounded-2xl p-6 shadow-lg flex flex-col md:flex-row items-center justify-between gap-6">
              <div>
                <h1 className="text-2xl sm:text-3xl font-bold">Performance Analytics, {firstName} </h1>
                    ...
                    <Tooltip 
                      ...
                      labelFormatter={(value, payload) => {
                        if (payload && payload.length) {
                          return `Quiz: ${payload[0].payload.name}`;
                        }
                        return value;
                      }}
                    />
              ...
              {analytics.weaknesses.length > 0 ? (
                <div className="space-y-3">
                  {analytics.weaknesses.map((subject, index) => (
                    <div key={subject.subject} className="flex items-center justify-between p-3 bg-amber-50 rounded-lg border border-amber-200">
                      <div className="flex items-center gap-3">
...
function KpiCard({ title, value, hint, trend }) {
  const trendColor = trend === "positive" ? "text-green-600" : 
                     trend === "negative" ? "text-red-600" : "text-gray-400";
      ...
      <div className="flex items-center justify-between mb-2">
        ...
        {trend && (
          <div className={trendColor}>
            {trend === "positive" ? <FaArrowUp /> : 
             trend === "negative" ? <FaArrowDown /> : null}
          </div>
        )}
      </div>
...
function normalize(items) {
  return items.map((i) => {
    ...
    const score =
      typeof i.score === "number" ? i.score : typeof i.result?.score === "number" ? i.result.score : null;
