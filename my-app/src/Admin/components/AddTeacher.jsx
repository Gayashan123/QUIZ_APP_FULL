...
import { FaUserTie, FaEnvelope, FaPhone, FaLock } from "react-icons/fa";
...
export default function AddTeacherForm({ onAddTeacher, onCancel, initialData, isEditMode, error, setError }) {
  ...
  return (
    <form className="bg-white shadow-xl rounded-2xl p-6 max-w-md mx-auto space-y-5" onSubmit={handleSubmit}>
      <h2 className="text-2xl font-semibold">{isEditMode ? "Update Teacher" : "Add New Teacher"}</h2>
      ...
      <div className="relative">
        ...
        <input type="password" name="password" value={formData.password} onChange={handleChange} placeholder={isEditMode ? "Leave blank to keep password" : "Password"}
          className="w-full pl-10 pr-4 py-3 border rounded-xl focus:ring-2 focus:ring-blue-400 focus:outline-none transition" />
      </div>

      {/* Buttons */}
      <div className="flex justify-end gap-3 pt-4">
        <button type="button" onClick={onCancel} className="px-5 py-2 rounded-xl bg-gray-100 hover:bg-gray-200 transition">Cancel</button>
