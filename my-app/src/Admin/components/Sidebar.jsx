...
//add the spinner 
  ...
  // Logout using Axios instance
  const handleLogout = async () => {
  setIsLoading(true);
  try {
    await api.get("logout"); // use GET instead of POST
  } catch (err) {
  ...
  const DesktopSidebar = (
    <aside
      className={`hidden md:flex flex-col ${collapsed ? "w-20" : "w-64"} bg-white/90 backdrop-blur border-r border-slate-200 p-4 sticky top-0 h-screen transition-all duration-300`}
    >
